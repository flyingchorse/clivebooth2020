...
// Modified site title for latest version of site as of 2020
function title_header2020 ($titletype){
	...
	if ($titletype == 1) {
		?><span id="post-title" class="navpostheader"><?php the_title(); ?></span>
			 <?php
				 }
				 else
				 {?>
						 
						<span class="navpostheader">
							<span id="post-title" class="subarticle"><?php the_title(); ?></span><span class="sub-article-wrap"><?php  ?><span class="sub-subarticle sub"></br></span></span>
						</span>
						<?php
				 }
	...
	function digidol_site_title_2020($titletype) {
		do_action('digidol_site_title_2020', $titletype);
	} // end digidol_site_title_2020
	
	add_action('digidol_site_title_2020','title_header2020', 10, 1);
...
function child_theme_setup() {
	...
	add_image_size( 'grid-packer', 90, 60, true );
	add_image_size( 'grid-packer-portrait', 90, 120, true );
...
function my_post_gallery( $output, $attr) {
    ...
    $output = apply_filters('gallery_style', "
        ...
        <div id='$selector' class='grid galleryid-{$id}'>");

    $i = 0;
    foreach ( $attachments as $id => $attachment ) {
        
        $imagethumbnailaspect = wp_get_attachment_image_src($id, 'full') ;
		$aspect_thumbnail =	($imagethumbnailaspect[2]>$imagethumbnailaspect[1] ? "grid-packer-portrait": "grid-packer");
		// $imagethumbnail = wp_get_attachment_image_src($attachment, $aspect_thumbnail) ;
$link = isset($attr['link']) && 'file' == $attr['link'] ? wp_get_attachment_link($id, $aspect_thumbnail, false, false) : wp_get_attachment_link($id, $aspect_thumbnail, true, false);
        $output .= "<div class='gallery-item grid-item " . ($aspect_thumbnail=="grid-packer-portrait" ? "grid-item--width2": "") . "'>";
        $output .= "
                $link
            ";
